...
const VideoScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40">
      <div className="container mx-auto px-2 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-4xl">
        {/* Back Button */}
        <Link
          to={`/courses/${courseId}`}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
                <div className="relative group">
                  <video
                    ref={videoRef}
                    src={video.videoUrl}
                    className="w-full aspect-video bg-slate-900 rounded-t-3xl"
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
                    {/* Play/Pause Overlay */}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <button onClick={() => skipTime(-10)} className="text-white hover:text-blue-300 transition-colors">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="relative">
                            ...
                            {showSettings && (
                              <div className="absolute bottom-8 right-0 bg-white/95 backdrop-blur-sm rounded-xl border border-slate-200/60 shadow-lg p-3 min-w-32">
                                <div className="text-sm font-medium text-slate-700 mb-2">Playback Speed</div>
                                {[0.5, 0.75, 1, 1.25, 1.5, 2].map(rate => (
                                  <button
                                    key={rate}
                                    onClick={() => changePlaybackRate(rate)}
                                    className={`block w-full text-left px-3 py-2 text-sm rounded-lg transition-colors ${playbackRate === rate
                ...
                {/* Video Info */}
                <div className="p-8">
                  <div className="inline-flex items-center gap-2 bg-blue-50/80 backdrop-blur-sm border border-blue-200/40 rounded-full px-4 py-2 mb-4">
                  ...
                  <h1 className="text-2xl sm:text-3xl lg:text-4xl font-medium text-slate-800 mb-6 leading-tight">
                    {video.title}
                  </h1>
                  ...
                  <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-200/40">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="text-lg font-medium text-slate-800">About this video</h3>
                    </div>
              ...
              <div className="flex gap-4">
                {previousVideo && (
                  <Link
                    ...
                    className="flex-1 bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 shadow-sm p-6 hover:shadow-md transition-all duration-300 group"
                  >
                    <div className="flex items-center gap-4">
            ...
            <div className="xl:col-span-1">
              <div className="sticky top-8">
                <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-slate-200/60 shadow-sm p-6">
                  <h3 className="text-lg font-medium text-slate-800 mb-6">Course Content</h3>

                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {content?.map((section, sectionIndex) => (
                      <div key={sectionIndex}>
                        <h4 className="text-sm font-medium text-slate-600 mb-2 px-3">
                          {section.sectionName}
                        </h4>
                        {section.videos.map((sectionVideo, videoIndex) => (
                          <Link
                            ...
                            className={`block p-3 rounded-xl transition-all duration-300 ${sectionVideo._id === videoId
                              ? 'bg-blue-50 border border-blue-200/60 text-blue-700'
                              : 'hover:bg-slate-50 text-slate-600'
                              }`}
                          >
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${sectionVideo._id === videoId
                                ? 'bg-blue-100'
                                : 'bg-slate-100'
                                }`}>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-red-50/80 backdrop-blur-sm rounded-3xl border border-red-200/60 p-8 text-center">
              ...
              <p className="text-red-700 text-lg font-medium">Video not found</p>
              <p className="text-red-600 text-sm font-light mt-1">The requested video could not be located</p>
            </div>
          </div>
        )}
      </div>
